...
class Transfer extends Base
{
    ...
    public static function expired()
    {
        ...
        if ($result) {
            ...
            foreach ($transfers as $transfer) {
                $editors = Staff::editors($transfer->transfer_to);
                if (empty($editors)) {
                    continue;
                }
    ...
    public static function paginate($page = 1, $perpage = 10, $search = null, $status = null)
    {
        ...
        $query = $query
            ...
            ->where(Base::table('transfers.status'), '=', 'transfer');
    ...
    protected static function buildQuery()
    {
        return Query::table(static::table())->left_join(
            ...
            's.id',
            '=',
            Base::table('transfers.staff_id')
        )->left_join(
            Base::table('staffs as a'),
            'a.id',
            '=',
            Base::table('transfers.transfer_by')
        )->left_join(
            Base::table('staffs as b'),
            'b.id',
            '=',
            Base::table('transfers.confirmed_by')
        )->left_join(
            Base::table('divisions as from'),
            'from.id',
            '=',
            Base::table('transfers.transfer_from')
        )->left_join(
            Base::table('divisions as to'),
            'to.id',
            '=',
            Base::table('transfers.transfer_to')
        );
    }
